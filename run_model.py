...
def main():
    ...
    #### AGENT POOLS ####
    base_params_eb = {
        'current_location': microhub_location,
        'delivery_hub_location': microhub_location,
        'speed': 10,
        'parking':norm(1/60,1/(120*6))}

    base_params_van = {
        'current_location': [0,20],
        'delivery_hub_location': [0,20],
        'speed': 20,
        'parking':norm(5/60,1/60)}
    ...
    electric_bike_pool = da.AgentPool(env=env, num_agents=20, agent=da.Electric_Bike, package_dist=poisson(30), turnaround_time=norm(1/120,1/(120*6)), turnaround_params=1/12, base_params=base_params_eb)
    courier_van_pool = da.AgentPool(env=env, num_agents=2, agent=da.Courier_Van, package_dist=poisson(300), turnaround_time=norm(1/120,1/(120*6)),turnaround_params=1, base_params=base_params_van)
    courier_vehicle_pool = da.AgentPool(env=env, num_agents=10, agent=da.Courier_Car, package_dist=poisson(50), turnaround_time=norm(1/120,1/(120*6)),turnaround_params=1, base_params=base_params_veh)
    ...
    location, building = sim_data.generate_demand(1000)
    env.process(courier_van_pool.process_deliveries(location, building))
    location, building = sim_data.generate_demand(1000)
    env.process(courier_vehicle_pool.process_deliveries(location, building))
